...
//       phase: "Initial",
//       message: "Tableau initial créé",
//       tableau: this.formatTableauForDisplay(tableau),
//     });
...
//         solution[`x${i + 1}`] = this.decimalToFraction(value);
//       } else {
//         solution[`x${i + 1}`] = value.toString();
//       }
//     }
...
//     // Convert objective value to fraction if needed
//     if (Math.abs(objectiveValue - Math.round(objectiveValue)) > 1e-6) {
//       solution.Z = this.decimalToFraction(objectiveValue);
//     } else {
//       solution.Z = objectiveValue.toString();
//     }
...
//   /**
//    * Format tableau for display with fractions
//    */
//   formatTableauForDisplay(tableau) {
//     return tableau.map(row => 
//       row.map(val => {
//         if (typeof val === 'number') {
//           if (Math.abs(val - Math.round(val)) > 1e-6) {
//             return this.decimalToFraction(val);
//           } else {
//             return val.toString();
//           }
//         }
...
//           row: leavingRow,
//           col: enteringCol,
//           value: this.decimalToFraction(tableau[leavingRow][enteringCol]),
//         },
//         tableau: this.formatTableauForDisplay(tableau),
//       });
...
//    */
//   printTableau(tableau) {
//     console.table(this.formatTableauForDisplay(tableau));
//   }
// }
...
class SimplexModel {
  ...
  solve(problem) {
    ...
    this.steps.push({
      phase: "Initial",
      message: "Problème converti en forme standard",
      tableau: this.formatTableauForDisplay(this.createInitialTableau(standardForm)),
    });
    ...
    // Print the solution
    console.log("\n✅ Final Solution:");
    console.table(this.formatSolutionForDisplay(solution));

    return {
      solution: this.formatSolutionForDisplay(solution),
      steps: this.steps,
    };
  }

  /**
   * Formate une solution pour l'affichage avec fractions
   * @param {Object} solution - Solution à formater
   * @returns {Object} - Solution formatée
   */
  formatSolutionForDisplay(solution) {
    ...
    for (const [key, value] of Object.entries(solution)) {
      if (typeof value === 'number') {
        formatted[key] = FractionUtils.formatNumber(value);
      } else {
        formatted[key] = value;
      }
    }
  ...
  /**
   * Formate un tableau pour l'affichage avec fractions
   * @param {Array} tableau - Tableau à formater
   * @returns {Array} - Tableau formaté
   */
  formatTableauForDisplay(tableau) {
    return FractionUtils.formatTableau(tableau);
  }
  ...
  simplexAlgorithm(standardForm) {
    ...
    this.steps.push({
      phase: "Initial",
      message: "Tableau initial créé",
      tableau: this.formatTableauForDisplay(JSON.parse(JSON.stringify(tableau))),
    });
    ...
    for (let i = 0; i < numOriginalVars; i++) {
      ...
      const column = tableau.map((row) => row[i]);
      const onePositions = column.reduce((positions, val, idx) => {
        if (Math.abs(val - 1) < 1e-10) positions.push(idx);
        return positions;
      }, []);
      ...
      if (
        onePositions.length === 1 &&
        column.filter((v) => Math.abs(v) > 1e-10 && Math.abs(v - 1) > 1e-10).length === 0
      ) {
        const basicRowIndex = onePositions[0];
        value = tableau[basicRowIndex][tableau[0].length - 1];
        isBasic = true;
    ...
    while (iteration < maxIterations) {
      ...
      this.steps.push({
        ...
        pivot: {
          row: leavingRow,
          col: enteringCol,
          value: FractionUtils.formatNumber(tableau[leavingRow][enteringCol]),
        },
        tableau: this.formatTableauForDisplay(JSON.parse(JSON.stringify(tableau))),
      });
  ...
  updateObjectiveRowForPhaseII(tableau) {
    ...
    for (let j = 0; j < tableau[0].length - 1; j++) {
      ...
      if (basicRowIndex !== -1 && Math.abs(tableau[objectiveRowIndex][j]) > 1e-10) {
        // Update objective row
        const coefficientToEliminate = tableau[objectiveRowIndex][j];
  ...
  findBasicVariableRow(tableau, col) {
    ...
    for (let i = 0; i < tableau.length - 1; i++) {
      if (Math.abs(tableau[i][col] - 1) < 1e-10) {
        oneCount++;
        oneRowIndex = i;
      } else if (Math.abs(tableau[i][col]) > 1e-10) {
        return -1; // Not a basic variable column
      }
    }
  ...
  pivot(tableau, pivotRow, pivotCol) {
    ...
    console.log(
      `🔄 Pivoting on element at row ${pivotRow + 1}, column ${
        pivotCol + 1
      } (value: ${FractionUtils.formatNumber(pivotElement)})`
    );
  ...
  /**
   * Print the tableau in a formatted way with fractions
   * @param {Array} tableau - Current tableau
   */
  // printTableau(tableau) {
  //   // Create a formatted tableau with fractions
  //   const formattedTableau = this.formatTableauForDisplay(tableau);
  //   console.table(formattedTableau);
  // }
  printTableau(tableau) {
    // Utiliser formatTableauForDisplay qui utilise maintenant toujours les fractions
    const formattedTableau = this.formatTableauForDisplay(tableau);
    console.table(formattedTableau);
}
}
