...
//       console.log('📝 Converting MIN problem to MAX by negating objective:', standardObjective);
//     }
...
//       console.log('⚠️ The problem is infeasible - artificial variables remain in the basis');
//     }
...
//         if (j >= numOriginalVars + numSlackVars + numSurplusVars && 
...
//       console.log('⚠️ Maximum iterations reached - solution may not be optimal');
//     }
//   }
...
//     console.log(`🔄 Pivoting on element at row ${pivotRow + 1}, column ${pivotCol + 1} (value: ${pivotElement})`);
...
//       row.map(val => typeof val === 'number' ? parseFloat(val.toFixed(4)) : val)
//     );
...
class SimplexModel {
  ...
  convertToStandardForm(objective, objectiveType, constraints) {
    ...
    if (objectiveType === 'min') {
      ...
      this.steps.push({
        phase: "Initial",
        message: "Conversion de problème MIN en MAX par négation de la fonction objective",
      });
    }
  ...
  phaseOne(tableau, standardForm) {
    ...
    while (iteration < maxIterations) {
      ...
      this.steps.push({
        ...
        pivot: { row: leavingRow, col: enteringCol, value: tableau[leavingRow][enteringCol] },
        tableau: JSON.parse(JSON.stringify(tableau)),
      });
    ...
    if (Math.abs(tableau[tableau.length - 1][tableau[0].length - 1]) > 1e-10) {
      ...
      this.steps.push({
        phase: "Phase I",
        message: "Le problème est non réalisable - variables artificielles restent dans la base",
      });
    }
